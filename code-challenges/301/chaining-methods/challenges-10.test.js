...
Write a function named count that, given an integer and an array of arrays, uses either filter, map, or reduce to 
count the amount of times the integer is present in the array of arrays.
...
const count = (target, input) => {
    ...
    let targetNums = allNums.filter((num) => num === target);
    return targetNums.length;

    // or this
    // return input.reduce((a,b)=> a.concat(b), []).filter((num) => num === target).length;
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((a,b)=> a.concat(b), []).reduce((accumulator, currentValue)=> accumulator + currentValue);
};
